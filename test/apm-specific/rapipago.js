...
describe('Rapipago', () => {
    it('should succeed Rapipago payment', async () => {
        ...
        nock('https://api.sandbox.checkout.com')
            .post('/apms/rapipago/payments/pay_b4wrbvczujuujja72omsufguzu/succeed')
            .reply(200);
        ...
        const rapipago = await cko.rapipago.succeed(payment.id);
    });

    it('should throw NotFoundError trying to succeed Rapipago payment', async () => {
        nock('https://api.sandbox.checkout.com')
            .post('/apms/rapipago/payments/1234/succeed')
            .reply(404);

        const cko = new Checkout(SK);

        try {
            const rapipago = await cko.rapipago.succeed('1234');
        } catch (err) {
            expect(err).to.be.instanceOf(NotFoundError);
        }
    });

    it('should expire Rapipago payment', async () => {
        nock('https://api.sandbox.checkout.com')
            .post('/payments')
            .reply(201, {
                id: 'pay_b4wrbvczujuujja72omsufguzu',
        ...
        nock('https://api.sandbox.checkout.com')
            .post('/apms/rapipago/payments/pay_b4wrbvczujuujja72omsufguzu/expire')
            .reply(200);
        ...
        const rapipago = await cko.rapipago.expire(payment.id);
    });

    it('should throw NotFoundError trying to expire Rapipago payment', async () => {
        nock('https://api.sandbox.checkout.com')
            .post('/apms/rapipago/payments/1234/expire')
            .reply(404);

        const cko = new Checkout(SK);

        try {
            const rapipago = await cko.rapipago.expire('1234');
        } catch (err) {
            expect(err).to.be.instanceOf(NotFoundError);
        }
    });
