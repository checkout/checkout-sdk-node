...
describe('Oxxo', () => {
    it('should succeed Oxxo payment', async () => {
        ...
        nock('https://123456789.api.sandbox.checkout.com')
            .post('/apms/oxxo/payments/pay_b4wrbvczujuujja72omsufguzu/succeed')
            .reply(200);
        ...
        const oxxo = await cko.oxxo.succeed(payment.id);
    });

    it('should throw NotFoundError trying to succeed Oxxo payment', async () => {
        nock('https://123456789.api.sandbox.checkout.com')
            .post('/apms/oxxo/payments/1234/succeed')
            .reply(404);

        const cko = new Checkout(SK, { subdomain: '123456789' });

        try {
            const oxxo = await cko.oxxo.succeed('1234');
        } catch (err) {
            expect(err).to.be.instanceOf(NotFoundError);
        }
    });

    it('should expire Oxxo payment', async () => {
        nock('https://123456789.api.sandbox.checkout.com')
            .post('/payments')
            .reply(201, {
                id: 'pay_b4wrbvczujuujja72omsufguzu',
        ...
        nock('https://123456789.api.sandbox.checkout.com')
            .post('/apms/oxxo/payments/pay_b4wrbvczujuujja72omsufguzu/expire')
            .reply(200);
        ...
        const oxxo = await cko.oxxo.expire(payment.id);
    });

    it('should throw NotFoundError trying to expire Oxxo payment', async () => {
        nock('https://123456789.api.sandbox.checkout.com').post('/apms/oxxo/payments/1234/expire').reply(404);

        const cko = new Checkout(SK, { subdomain: '123456789' });

        try {
            const oxxo = await cko.oxxo.expire('1234');
        } catch (err) {
            expect(err).to.be.instanceOf(NotFoundError);
        }
    });
