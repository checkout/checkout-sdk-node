...
describe('Integration::Platforms::Reserve Rules', () => {
    ...
    it('should update a reserve rule', async () => {
        const getResponse = await cko_platforms.platforms.getReserveRuleDetails(entityId, reserveRuleId);
        const etag = getResponse.headers?.etag;

        expect(etag).to.not.be.undefined;

        const response = await cko_platforms.platforms.updateReserveRule(
            entityId,
            reserveRuleId,
            {
                type: 'rolling',
        ...
        expect(response).to.not.be.null;
        expect(response.id).to.equal(reserveRuleId);
        expect(response._links).to.not.be.null;
        expect(response.headers?.etag).to.not.be.undefined;
        
        // Verify update by fetching again
        const updatedResponse = await cko_platforms.platforms.getReserveRuleDetails(entityId, reserveRuleId);
        expect(updatedResponse.rolling.percentage).to.equal(15);
        expect(updatedResponse.rolling.holding_duration.weeks).to.equal(4);
    });
